...
terraform {
  cloud {
    organization = "terraform-prueba-ami" 
    workspaces {
      name = "monorepo-backend" # <--- ASEGÚRATE DE CREAR ESTE WORKSPACE
    }
  }
}
...
resource "aws_security_group" "instancia_sg" {
  name        = "inst-sg-back" # Nombre único
  vpc_id      = data.aws_vpc.default.id
...
resource "aws_launch_template" "mi_template" {
  ...
  user_data = base64encode(<<-EOF
              ...
              # CLONAR EL MONOREPO
              git clone https://github.com/mariatashiguano/terraform-ami-prueba.git .
